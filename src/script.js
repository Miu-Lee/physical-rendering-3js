...
const updateAllMaterials = () =>{
    scene.traverse((child)=>{
        if(child instanceof THREE.Mesh && child.material instanceof THREE.MeshStandardMaterial){
            child.material.envMap = envriomentMap
 ...
 * Environment Map
 */
const envriomentMap = cubeTextureLoader.load([
...
scene.background = envriomentMap;
envriomentMap.encoding = THREE.sRGBEncoding;
//scene.environment = envriomentMap;
debugObject.envMapIntensity = 3
gui.add(debugObject, 'envMapIntensity').min(0).max(5).step(0.01).onChange(updateAllMaterials)
...
gltfLoader.load(
    ...
    (gltf)=>{
        ...
        gui.add(gltf.scene.rotation, 'y').min(-Math.PI).max(Math.PI).step(0.01).name('rotate');
        updateAllMaterials();
    }
)
